...
int main(int argc, char* argv[]) {
    if (argc < 4 || strcmp(argv[1], "--f") != 0) {
        printf("ejemplo: %s --f {blur|laplace|sharpen} img1 img2 img3 ...\n", argv[0]);
        return 1;
    }

    const char* filterType = argv[2];
    Filter f(filterType);
    ...
    for (int i = 0; i < numImages; i++) {
        std::string inputName = images[i];
        std::string outputName = "out_" + inputName;
